...
export function WorkerInviteForm({ token }: { token: string }) {
    ...
    async function loadData() {
      try {
        ...
        if (invRes.error) {
          ...
          setInvitation(invRes.data as InvitationData);
        }

        if (sessRes.data?.user) {
          setSessionUser({ email: sessRes.data.user.email });
        }
      } catch {
  ...
  const renderSessionActions = (invite: InvitationData) => {
    if (!sessionUser) {
      return (
        <div className="space-y-4">
          <div className="rounded-md border border-slate-200 bg-slate-50 p-4 text-slate-700 text-xs leading-relaxed">
            ...
            <span className="font-semibold">{invite.email}</span> to accept this
            invitation.
          </div>
